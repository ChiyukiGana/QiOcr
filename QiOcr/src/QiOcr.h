...
template <class Ty>
Ty clamp(const Ty& val, const Ty& min, const Ty& max)
{
	Ty _min = std::min(min, max);
	Ty _max = std::max(min, max);
	if (val < _min) return min;
	if (val > _max) return max;
	return val;
}
...
{
public:
	int init(void* modelData, size_t modelSize, size_t threads = 0)
	{
		OcrBase::release();
		threads = clamp(threads, 0ull, 32ull);
...
public:
	int init(void* modelData, size_t modelSize, const std::vector<std::string>& keys, size_t threads = 0, size_t scaleSize = 48)
	{
		OcrBase::release();
		m_keys = keys;
	...
	int init(void* modelData, size_t modelSize, void* keysData, size_t keysSize, size_t threads = 0, size_t scaleSize = 48)
	{
...
public:
	...
	{
		std::vector<std::string> result = scan_list(image, skipDet);
		std::string text;
	...
	{
		std::vector<std::string> result = scan_list(rect, skipDet);
		std::string text;
		for (const std::string& i : result)
		{
			if (text.empty()) text += i;
